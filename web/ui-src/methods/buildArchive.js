  buildArchive(){
    ...
    const fmtScore = (v, scaleMax) => scaleMax === 10 ? (Math.round(v * 10) / 10).toFixed(1) : String(Math.round(v));
    ...
    const traits = mine.list.map((score, i) => {
      ...
      return {
        label: TLBL[i], score: fmtScore(score, mine.scaleMax), desc: m.desc,
    ...
    const weekCards = WEEKS.map((c, ci) => {
      ...
      const axes = c.scores.map((sc, i) => {
        ...
        const lx = (CX + R * 1.32 * Math.cos(a)) / 160 * 100, ly = (CY + R * 1.32 * Math.sin(a)) / 160 * 100;
        return {
          label: TLBL[i], score: fmtScore(sc, c.scaleMax), ax: outer[0], ay: outer[1], dx: dot[0], dy: dot[1],
    ...
    const cardSheet = {
      ...
      rows: sw.scores.map((sc, k) => ({
        label: TLBL[k], score: fmtScore(sc, sw.scaleMax), note: TMETA[k].desc,

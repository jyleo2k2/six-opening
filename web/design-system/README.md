...
- **빌드에 들어가지 않는다.** 실제 화면 원본은 `web/ui-src`이고 `scripts/ui-build.mjs`가
  `public/ui/app.html`로 조립한다. 이 폴더는 그 파이프라인과 무관한 참조용이다.
- **손으로 고치지 않는다.** 디자인 변경은 claude.ai/design 쪽에서 하고 여기로 다시 받아 온다.
  반대로 여기서 고치면 다음 반입 때 사라진다.

## 반영 경로

이 폴더의 토큰·컴포넌트를 화면에 적용할 때는 `web/ui-src`의 해당 조각을 고친다.

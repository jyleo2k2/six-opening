...
**시즌 경계를 아는 API가 아직 없다**(`season-day.ts` 머리말). `/api/family` 가 구성원마다 준 주차 카드(`weeks`, 진행 중인 이번 주 포함)를 두 탭이 나눠 쓴다 — 표본 픽스처(`LAST_SEASON`)는 2026-08-17 지웠고 화면은 늘 서버 값만 읽는다.

- **이번 시즌**(`thisSeasonWeeks`) — 구성원 한 명의 주차 카드 **전체**(끝난 주 + 진행 중인 이번 주)를 화면 모양으로 편다. `우리 가족 투자` 탭에서 구성원을 누르면 이 값으로 주차별 흐름을 펼친다.
- **지난 시즌**(`closedWeekRows` → `seasonReport`) — **끝난 주(`status: "closed"`)만** 걸러 리포트를 만든다. 한 사람의 시즌 성향은 **그 사람 주차 유형 중 최빈값**, 가족의 시즌 성향은 **구성원 시즌 성향 중 최빈값**이다. 동점이면 입력 순서가 앞선 쪽을 고른다 — `Object.keys` 순서에 맡기면 같은 입력이 브라우저마다 다른 유형을 낸다. **점수를 다시 매기지 않는다.** 채점은 `shared/engine/behavior-profile.ts` 하나가 한다.

두 탭 다 **진짜 시즌 경계로 자르지 않는다** — 지금 DB 에는 이번 시즌(2026-08-03~) 기록만 있어서다. `지난 시즌` 은 사실 "끝난 주차 모음"이고, 시즌이 여러 번 쌓이면 자연히 진짜 지난 시즌 주차만 남는다. 경계로 좁히는 것은 그게 실제로 필요해질 때 한다(§7).
...
| 우리가족투자 — 지난 시즌 | `members[].weeks`에서 끝난 주만(§5.2 `closedWeekRows`·`seasonReport`) | 되짚을 끝난 주가 없다는 빈 자리 문구 |
...
- **`지난 시즌` 은 진짜 시즌이 아니라 "끝난 주차 모음"이다.** 서버에 시즌 경계 API 가 아직 없어서다(§5.2). 로그인한 가족의 실제 `/api/family` 주차 카드를 쓰므로 값 자체는 진짜지만, 여러 시즌이 지나기 전에는 이번 시즌 안에서 끝난 주만 보게 된다. 시즌 경계로 좁히는 것은 그게 실제로 필요해질 때 한다.

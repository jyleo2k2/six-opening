...
| **우리가족투자** | `/archive/family`(현시즌) · `/archive/last`(지난시즌) | 가족 구성원 오각형 겹치기. 머리의 둘째 줄로 현시즌·지난시즌을 **한 페이지에서 갈아 끼운다** |
| **우리가족 수익** | `/archive/return` | 머리 요약 카드(총자산·손익·상세), 가족 달리기 트랙, 거래 피드 |

머리의 탭 줄(`우리가족투자`·`우리가족 수익`)은 **가족 것 둘뿐**이다. 첫 화면은 내 것이라 어느 탭도 켜지지 않으며, 돌아오는 길은 머리의 뒤로가기(`‹`)다. 첫 화면에서 `‹` 를 누르면 홈으로 나간다.

...
> **통합문서 v2 와의 관계**: §21 레드라인의 해당 줄은 `AI의 종목 추천·매매 시점·목표가·수익률 전망·훈계` 로 **AI 출력을 겨눈다**. 이 시트는 화면이 갖는 고정 목록이고 LLM 은 관여하지 않으므로 그 줄에 걸리지 않는다 — 2026-08-17 판단이고 통합문서는 고치지 않았다. **LLM 이 종목을 말하는 길(F10·`shared/llm`)은 그대로 막혀 있다.**

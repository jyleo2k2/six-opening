...
| 자리 | 주소 | 내용 |
|---|---|---|
| **첫 화면 — 성장 아카이브** | `/archive`, `/archive/cards` | 로그인한 사람 **개인**의 주차 성향 카드를 가로 레일로 깐다. 옆으로 넘겨 지난 주를 보고, 가운데 카드를 누르면 축 상세 시트가 열린다. 제목 오른쪽에 **내 총자산·손익·수익률**이 붙는다 |
| **우리가족투자** | `/archive/family`(현재 시즌) · `/archive/last`(과거 시즌) | 가족 구성원 오각형 겹치기. `현재 시즌`·`과거 시즌` 줄로 **한 페이지에서 갈아 끼운다** |
| **우리가족 수익** | `/archive/return` | 머리 요약 카드(총자산·손익·상세), 가족 달리기 트랙, 거래 피드 |

**가족 단추 둘(`우리가족투자`·`우리가족 수익`)은 첫 화면에만 있다.** 탭이 아니라 문이라 켜짐·꺼짐이 없고 늘 분홍(`#D70082`)이며, 누르면 그 자리로 들어가고 단추 줄째로 사라진다. 돌아오는 길은 머리의 뒤로가기(`‹`) 하나이고, 첫 화면에서 `‹` 를 누르면 홈으로 나간다.

**지금 어디인지는 제목이 말한다** — `성장 아카이브` · `우리가족투자` · `우리가족 수익`. 제목 위에 있던 회색 머리말 줄은 없앴다.

우리가족투자의 시즌 줄(`현재 시즌`·`과거 시즌`)은 첫 화면 가족 단추와 **같은 자리·같은 크기**를 쓴다. 한 폰 안에서 줄마다 단추 높이가 다르면 안 된다.
...
- **첫 화면 제목 옆 지갑은 가족 합계가 아니라 내 계좌다.** `/api/family` 는 자산 규모를 마스킹해 구성원의 평가금액·원금·현금을 **주지 않고 `returnRate` 만** 준다(F11 SPEC §5·`app/api/family/route.ts` 주석). 그래서 가족 총자산은 화면에서 낼 수 없고 라벨을 `내 총자산` 으로 적었다. 가족 합계를 띄우려면 서버가 합계를 새로 내려야 하는데, **2인 가족이면 합계에서 내 것을 빼 상대 자산이 그대로 드러난다** — 마스킹 규칙을 바꾸는 결정이 먼저다.

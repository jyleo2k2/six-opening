...
- **주문 화면 1단계의 좌측 상단 X는 이 시트로 되짚는다**(2026-08-18). 종목 상세가 아니라 눌렀던 그 시트(같은 주차 카드·같은 유형 목록)를 다시 열며 첫 화면으로 돌아간다 — 시트를 보다 담을지 말지 정하러 나간 것이지 그 종목을 알아보러 간 것이 아니기 때문이다. 자리는 주소로 표현하지 않고 `ConnectedPrototype`이 `f0-home/CLAUDE.md`에 적은 대로 메모리 상태로 물려 준다.
...
**카드 머리의 시각은 글을 올린 때다 (2026-08-18).** 이름 밑 한 줄(`3시간 전`)과 카드 순서가 `transactions.feed_posted_at`(`/api/family` 가 `feedPostedAt` 으로 준다)으로 난다. 예전에는 체결 시각이라 지난주에 산 종목을 오늘 올려도 방금 쓴 글이 `5일 전` 으로 뜨고 남의 옛 글 밑에 묻혔다 — 피드는 2026-08-17 부터 체결과 따로 논다(`feed_body` 를 써야 올라간다). **언제 산 것인지는 색 판 맨 위 날짜 라벨(`8월 13일 매수`)이 그대로 말한다.** 마이그레이션 전에 올라가 있던 글만 값이 없어 서버가 체결 시각으로 접어 준다. 가드: `f0-home/lib/archive-view.test.ts`.
...
**본문은 피드에 올릴 때 쓴 글(`feed_body`) 하나뿐이다 (2026-08-18).** 주문할 때 남긴 `memo`, 고른 이유, 보유 계획·목표가, 계획 변경 이유를 본문 뒤에 이어 붙이지 않는다 — `피드에 올리기` 시트가 "가족에게 남길 말" 을 따로 받는데 그 뒤에 며칠 전 주문 화면에서 고른 문구가 저절로 따라붙으면, 자기가 쓰지도 않은 말이 자기 글로 읽힌다. 계획·목표가는 오른쪽 회색 판이, 고른 이유는 `shortMent` 가 이미 제 자리로 들고 있으니 본문에서 한 번 더 말할 이유도 없다. `feed_body` 가 비면 본문은 빈 줄이고 **화면이 메모·이유로 대신 채우지 않는다** — 서버가 `feed_body is not null` 로 거르므로 실제로는 오지 않는다. 접두사 `담았어. `·`팔았어. ` 도 붙이지 않는다 — 바로 위 날짜 라벨이 이미 매수·매도를 말한다. 가드: `f0-home/lib/archive-view.test.ts`.
...
**되돌아온 아카이브는 지난번 화면부터 그린다** (2026-08-18, `use-archive-data.ts`). `ArchiveScreen` 은 다른 화면으로 나가면 통째로 언마운트되므로(`ConnectedPrototype` 이 주소에 맞는 화면 하나만 그린다) 훅도 매번 다시 마운트된다. 예전에는 그때마다 상태가 빈칸에서 시작해, 화면 하나 다녀오는 값이 매번 `기록을 불러오고 있어요` + API 두 벌이었다.

- 지난번 `season`·`family`·댓글·좋아요를 모듈 메모리에 남기고(`readArchiveSnapshot`·`writeArchiveSnapshot`) 첫 상태를 거기서 세운다. 화면에서 바꾼 것(좋아요·댓글·글 올리기)도 함께 남아 되돌아왔을 때 방금 한 일이 사라져 보이지 않는다.
- **그래도 마운트마다 서버에 다시 묻는다.** 남겨 둔 화면은 먼저 그리려고 들고 있는 것이지 최신이라고 믿는 값이 아니다 — 주문하고 돌아온 화면이 옛 기록을 보여 주면 안 된다. 피드는 첫 50건으로 갈아 끼우고 더 내려 읽어 둔 뒷장은 버린다(되돌아온 화면은 맨 위부터 본다).
- 안내 문구는 **지난번 카드가 있을 때만** 건너뛴다(`!seeded?.season`). 첫 응답 전에 나갔다 온 빈 화면을 받아 둔 것으로 치면 위의 깜빡임(중립 카드 → 진짜 카드)이 그대로 돌아온다.
- 브라우저 저장소를 쓰지 않으므로 수명은 `candle-tip` 의 닫힘 표시와 같다 — **로그인 세션의 수명**이다. 로그아웃은 `/` 로 나가며 문서를 새로 받아 함께 비워진다. 가드는 `lib/archive-snapshot.test.ts`.

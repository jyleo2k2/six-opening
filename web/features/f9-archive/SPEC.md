...
| 자리 | 주소 | 내용 |
|---|---|---|
| **첫 화면 — 내 투자 성향** | `/archive`, `/archive/cards` | 로그인한 사람 **개인**의 주차 성향 카드를 가로 레일로 깐다. 옆으로 넘겨 지난 주를 보고, 가운데 카드를 누르면 축 상세 시트가 열린다 |
| **우리가족투자** | `/archive/family`(현시즌) · `/archive/last`(전시즌) | 가족 구성원 오각형 겹치기. `현시즌`·`전시즌` 줄로 **한 페이지에서 갈아 끼운다** |
| **우리가족 수익** | `/archive/return` | 머리 요약 카드(총자산·손익·상세), 가족 달리기 트랙, 거래 피드 |

**가족 단추 둘(`우리가족투자`·`우리가족 수익`)은 첫 화면에만 있다.** 누르면 그 자리로 들어가고 단추 줄은 사라진다 — 켜진 탭으로 남지 않는다. 지금 어디인지는 제목 위 머리말이 말하고(`우리가족투자 > 전시즌`), 돌아오는 길은 머리의 뒤로가기(`‹`) 하나다. 첫 화면에서 `‹` 를 누르면 홈으로 나간다.

우리가족투자의 시즌 줄(`현시즌`·`전시즌`)은 첫 화면 가족 단추와 **같은 자리·같은 크기**를 쓴다. 한 폰 안에서 줄마다 단추 높이가 다르면 안 된다.

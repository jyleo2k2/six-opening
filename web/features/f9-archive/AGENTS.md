...
- 단일 스펙: `web/features/f9-archive/SPEC.md`. 화면은 이 폴더가 아니라 `web/features/f0-home/ArchiveScreen.tsx`와 그 계산 함수(`lib/archive-profile-view.ts`·`lib/archive-feed.ts`·`lib/use-archive-data.ts`)에 있다. 이 폴더에는 `SPEC.md`와 화면 계약 가드(`lib/archive-feed-db-ui.test.ts`)만 둔다.

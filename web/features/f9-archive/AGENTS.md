...
- **그 시트에 목표가·수익률 전망·매매 시점을 붙이지 않는다.** 종목을 말하는 주체는 화면의 고정 목록뿐이고 **LLM이 종목을 말하는 길은 여전히 막혀 있다**(루트 가드). 통합문서 v2 §21의 레드라인은 `AI의 종목 추천`으로 AI 출력을 겨누므로 이 목록에는 걸리지 않는다 — 통합문서는 고치지 않았다(SPEC §1.1).

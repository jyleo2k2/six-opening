...
export function ExploreScreen() {
  ...
  return (
    <PhoneShell>
      ...
      <section className="bg-navy px-5 pb-6">
      ...
      <section className="flex-1 px-4 py-4">
        <div className="no-scrollbar flex gap-2 overflow-x-auto pb-3">{sectors.map((item) => <Chip key={item} selected={sector === item} onClick={() => setSector(item)}>{item}</Chip>)}</div>
        <div className="my-3 flex items-center justify-between text-xs opacity-60"><span>기업 목록 · {filtered.length}개</span><span className="flex items-center gap-1"><i className={`h-2 w-2 rounded-full ${live ? "bg-up" : "bg-gray"}`} />{live ? "키움 시세 반영" : "데모 시세"}</span></div>
        <div className="overflow-hidden rounded-2xl bg-white px-3 shadow-card">
          {filtered.map((stock) => {

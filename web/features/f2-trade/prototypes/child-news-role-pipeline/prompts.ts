...
headline, homeSummary, body 첫 줄은 모두 selection.anchorSourceId의 중심 사건부터 말한다. 회사 기사라면 선택된 회사 이름과 새 사건을 먼저 쓴다. 시장 기사라면 오늘 관측된 시장 움직임부터 쓴다. 주변 사례가 중심보다 앞서면 안 된다.

body는 서비스 상세 화면에 그대로 보이는 3줄 요약이다. 정확히 세 항목을 core_event, business_connection, context 순서로 쓰고, 각 text는 공백과 문장부호를 포함해 36자 이하여야 한다. 한 항목은 화면에서 최대 두 줄로 읽을 수 있는 한 문장만 쓴다.

세 줄은 서로 다른 정보를 맡아야 한다. 첫 줄은 무슨 일이 있었는지, 둘째 줄은 핵심 숫자·날짜·회사 연결 중 하나, 셋째 줄은 어려운 용어 설명이나 남은 중요 사실을 쓴다. 같은 사실을 어미만 바꾸거나 넓게 말한 뒤 숫자를 붙여 다시 반복하지 말고 homeSummary 문장도 body에 그대로 복사하지 마라. 예를 들어 둘째 줄에서 삼성전자·SK하이닉스 상승률을 쓸 거라면 첫째 줄에는 두 회사가 올랐다는 말을 넣지 말고 코스피의 움직임만 쓴다. 세 줄을 채우려고 원문에 없는 배경·평가·전망을 만들지 마라.
...
코스피가 선별 문장에 있으면 '국내 주식시장 숫자'처럼 이름을 지우지 마라. 노출문에 코스피를 그대로 쓰고 별도 요약 한 줄에서 '코스피는 국내 주식시장을 대표하는 숫자야.'처럼 이름과 뜻을 같은 문장에 써라. 이때 termTreatments는 term='코스피', treatment='explained'로 두고 easyText도 실제 노출문에 들어간 쉬운 설명이어야 한다.
...
selection.difficultTerms의 모든 항목을 termTreatments에서 정확히 한 번 처리한다. replaced는 노출문 전체에서 더 쉬운 말로 바꾼 경우, explained는 원래 용어가 필요해 easyText로 뜻을 풀어 쓴 경우다. easyText는 기록용 메모나 숨겨진 홈 요약이 아니라 서비스에 표시되는 body 3줄 중 한 곳에 실제 문구로 들어가야 한다. 어려운 용어 하나만 고르지 말고 남아 있는 어려운 표현을 전부 처리한다.
...
draft의 제목·홈 요약·첫 문장이 독립적으로 찾은 중심 사건과 맞는지, 주변 사실이 앞서지 않는지, 모든 주장과 숫자가 원문에 있는지, 주장·계획·시점의 귀속이 유지됐는지 검사한다. body가 정확히 세 줄이고 각 줄이 36자 이하인지, 세 줄이 같은 사실을 넓은 설명과 숫자 설명으로 되풀이하지 않는지도 conciseThreeLineSummary로 검사한다.

남은 금융·회계·정책·산업 용어와 약어를 10~13세가 이해할 수 있게 모두 바꾸거나 설명했는지도 검사한다. explained의 쉬운 설명은 서비스에 표시되는 body 3줄 안에 보여야 한다. 원문에 코스피가 있으면 body 한 줄 안에 코스피라는 이름과 '국내 주식시장을 대표하는 숫자'라는 뜻이 함께 있어야 한다. %, 억원, 조원, 1~4분기 표기가 남았다는 이유만으로 allTermsEasy를 실패시키지 마라.

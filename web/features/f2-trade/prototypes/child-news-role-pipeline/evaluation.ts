...
function renderServiceNewsCard(
  ...
  return `<article class="service-news-card" data-case-id="${escapeHtml(item.caseId)}">
    ...
    <div class="summary-panel"><strong>한눈에 보기</strong><p>${escapeHtml(draft.homeSummary.text)}</p></div>
    <div class="copy-panel">${body}<p class="byline">${escapeHtml(article.publisher)} · ${escapeHtml(article.runDateKst)} <a href="${safeHref(article.sourceUrl)}" target="_blank" rel="noreferrer">원문 보기</a></p></div>
    <div class="safety-note"><span aria-hidden="true">🐻</span><p>이 뉴스는 무엇을 사거나 팔라고 권하는 글이 아니에요. 오늘 어떤 일이 있었는지만 알려줘요.</p></div>
  </article>`;
}

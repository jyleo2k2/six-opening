...
function renderServiceNewsCard(
  ...
  return `<article class="service-news-card" data-case-id="${escapeHtml(item.caseId)}">
    ...
    <div class="summary-panel"><strong>3줄 요약</strong><ol class="three-line-summary">${summaryLines}</ol><p class="byline">${escapeHtml(article.publisher)} · ${escapeHtml(article.runDateKst)} <a class="source-link" href="${safeHref(article.sourceUrl)}" rel="noreferrer">원문 보기</a></p></div>
  </article>`;
}

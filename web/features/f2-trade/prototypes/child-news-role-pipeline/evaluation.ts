...
function renderServiceNewsCard(
  ...
  return `<article class="service-news-card" data-case-id="${escapeHtml(item.caseId)}">
    ...
    <div class="headline-panel"><span>${escapeHtml(storyLabel)} · 짧은 뉴스 카드</span><h2>${escapeHtml(draft.headline.text)}</h2></div>
    <div class="summary-panel"><strong>자세히 보기 · 같은 제목</strong><h2 class="detail-headline">${escapeHtml(draft.headline.text)}</h2><strong>3줄 요약</strong><ol class="three-line-summary">${summaryLines}</ol><div class="price-panel"><strong>왜 주가와 관련 있어?</strong><p>${escapeHtml(draft.priceConnection.text)}</p><small>${draft.priceConnection.basis === "article_fact" ? "기사에서 확인된 연결" : "사건 유형에 따른 교육 설명"}</small></div>${terms}<p class="byline">${escapeHtml(article.publisher)} · ${escapeHtml(article.runDateKst)} <a class="source-link" href="${safeHref(article.sourceUrl)}" rel="noreferrer">원문 보기</a></p></div>
  </article>`;
}

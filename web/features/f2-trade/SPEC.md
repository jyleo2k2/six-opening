...
적는 자리만 있고 **보이는 자리가 없었기 때문이다.** 매도 회고 카드의 `그때 한 말`은 §4.3
...
| `GET /api/trades` | 같은 필드를 함께 준다. 매도 2단계 회고 카드의 `그때 한 말`이 여기서 **매수** `memo`를 읽는다. 차트 마커 계산(`ChartTrade`)은 이 값을 쓰지 않는다 |

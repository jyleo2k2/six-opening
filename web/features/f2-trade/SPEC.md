...
2. **사던 날의 나**: 연결된 최근 매수 기록과 수익률을 보여 주고 매도 이유를 고른다. 카드 맨 위 큰따옴표 인용문은 **매수 때 직접 쓴 한 줄 메모**를 그대로 읽고, 메모가 없을 때만 이유·계획 문구로 만든 `… 샀어요.` 문장을 대신 쓴다 — 같은 말을 인용문과 아래 목록에 두 번 두지 않으려고 `그때 한 말` 행은 2026-08-18에 걷어냈다. 처음 계획과 다르면 변경 이유도 필수다. 매수 기록의 원본은 **`GET /api/trades?symbol=` 응답 중 `mine` 인 체결**이고 그 가운데 가장 최근 매수를 쓴다. 첫 매도 판정은 **그 매수 이후에 내 매도가 있었는지**로 본다 — 서버에는 매수·매도를 잇는 칸이 없다.

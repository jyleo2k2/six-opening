...
- **지정가는 종목의 호가단위에 맞춘 값만 접수한다.** 규칙은 `shared/engine/tick-size.ts` 하나가 소유하고 화면(`buyMath`·`sellMath`)·API(`POST /api/orders`)·DB(`reserve_order`) 세 곳이 같은 표를 본다. 구간은 KRX 현행(2023-01-25 개정)이고 **KOSDAQ은 5만원 이상이 100원으로 고정**돼 20만원부터 KOSPI(500원)와 갈린다 — 시장 구분의 원본은 `shared/data/stocks.ts`의 `market`이다. **스냅 방향은 매수 내림·매도 올림이다**: 아이가 `-3%`를 눌렀는데 실제가 -2.9%가 되면 누른 것보다 불리해진다. 다만 `지금값`(0%)은 스냅하지 않는다 — 현재가는 이미 유효 호가이고, 한 틱 내리면 체결 가능성만 떨어진다.
...
- **주문 화면 1단계 좌측 상단 X는 보통 그 회사 상세로 물러난다.** 다만 아카이브 추천종목 시트(F9 SPEC §1.1)에서 눌러 들어온 경우는 예외로, 종목 상세가 아니라 그 시트가 열린 아카이브 화면으로 돌아간다(2026-08-18) — 자세한 배선은 `f0-home/CLAUDE.md`를 본다.
...
2. **사던 날의 나**: 연결된 최근 매수 기록과 수익률을 보여 주고 매도 이유를 고른다. 카드 맨 위 큰따옴표 인용문은 **매수 때 직접 쓴 한 줄 메모**만 싣고, 메모가 없으면 인용문 줄을 통째로 그리지 않는다 — 이유·계획은 바로 아래 `왜 샀는지`·`언제까지` 행이 이미 말하므로 그것을 `… 샀어요.` 문장으로 바꿔 넣으면 카드가 같은 말을 두 번 한다. 같은 이유로 `그때 한 말` 행도 2026-08-18에 걷어냈다. 예외는 매수 기록 자체가 없을 때뿐이다: 아래 행도 함께 비므로 `기록이 없어요.` 를 인용문 자리에 남긴다. 처음 계획과 다르면 변경 이유도 필수다. 매수 기록의 원본은 **`GET /api/trades?symbol=` 응답 중 `mine` 인 체결**이고 그 가운데 가장 최근 매수를 쓴다. 첫 매도 판정은 **그 매수 이후에 내 매도가 있었는지**로 본다 — 서버에는 매수·매도를 잇는 칸이 없다.
3. **매도 완료**: 정규장 시장가는 체결, 장외 시장가는 다음 거래일 시가 예약, 지정가는 대기 주문으로 표시한다. **한 줄 메모는 없다** — 매수와 달리 다시 보여 줄 자리가 없어 2026-08-18에 걷어냈다(§7.2).
...
- 응답의 `transaction_id`는 방금 남긴 기록의 id다. 화면은 완료 표시에만 쓰고 뒤이어 고치지 않는다(§7.2).
- **확신도·자신감 수치는 이 요청 본문에도 없다** (§5.4).

### 7.2 매도 완료 화면에는 메모가 없다 (2026-08-18 철거)

**매도 메모는 화면·API·DB에서 모두 걷어냈다.** `PATCH /api/trade`, `set_trade_memo` RPC,
`SellDraft.memo`, 완료 화면의 입력칸과 `저장하기` 버튼이 함께 사라졌고, 이미 저장돼 있던
매도 메모는 `20260818082500_drop_sell_memo.sql` 이 `null` 로 비웠다.

적는 자리만 있고 **보이는 자리가 없었기 때문이다.** 매도 회고 카드의 인용문은 §4.3
2단계에서 **매수** 메모를 읽고, 가족 피드 본문은 `feed_body` 가 원본이라 `GET /api/family` 가
`feed_body is not null` 로 걸러 내보낸다. 매도 메모를 그리는 소비자는 처음부터 없었다.

- 새 매도 메모 입력·저장 경로를 되살리지 않는다. 필요해지면 **먼저 읽는 화면을 정하고** 이
  절부터 다시 쓴다.
- `transactions.memo` 컬럼은 남는다 — 매수 메모(§7.1)가 같은 칸을 쓰고 그쪽은 그대로다.
...
| `GET /api/trades` | 같은 필드를 함께 준다. 매도 2단계 회고 카드의 인용문이 여기서 **매수** `memo`를 읽는다. 차트 마커 계산(`ChartTrade`)은 이 값을 쓰지 않는다 |

...
> = {
  sniper: {
    ...
    // 저격수만 문장 사이를 `\n` 대신 띄어쓰기로 둔다(2026-08-18 유저 요청). 첫 문장이
    // 다른 유형보다 길어 손으로 끊으면 그 줄이 카드 폭에서 다시 접혀 석 줄이 됐다.
    desc: "찾아볼 건 다 찾아보고, 확신이 선 소수 섹터에 몰아 담았어요. 근거는 촘촘하지만 한쪽으로 쏠려 있어요.",
...
export type FamilyRow = {
   ...
   * 이번 시즌·지난 시즌 리포트(`archive-season.ts`)가 내 카드와 같은 코드로 편다.
   *

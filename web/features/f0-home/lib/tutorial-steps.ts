 ...
 *
 * **하필 크래프톤인 이유는 매도까지 가야 하기 때문이다.** 튜토리얼 뒷부분(14~18장)은
 * 파는 과정이고, 팔려면 이미 갖고 있어야 한다. 매수 장에서 넣은 주문은 장이 열려 있을
 * 때만 즉시 체결되므로 그것만 믿으면 장 밖에서는 매도 화면이 `판매할 주식이 없어요` 로
 * 막힌다. 크래프톤은 자녀 계정(김찬영)의 시드 보유라(`seed_family_portfolios`, 23주)
 * 시간과 무관하게 팔 수 있다. 게임 회사라 아이에게 설명하기도 쉽다.
 *
 * 여기 한 곳만 원본이다. 매도는 `:code` 로 지금 보고 있는 종목을 따라가므로 자동으로
 * 같은 회사가 된다.
 */
export const TUTORIAL_STOCK = "259960";

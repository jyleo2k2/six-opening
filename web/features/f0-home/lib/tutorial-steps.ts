 ...
 *
 * 흐름은 **환영 인사 → 홈 → 모의투자 → 섹터 → 종목 카드 → 상세(차트·회사·뉴스) →
 * 매수 3단계 → 매도 3단계** 다. 사고파는 과정을 한 번씩 끝까지 보여 주는 것이 목적이라
 * 계좌·랭킹·아카이브는 넣지 않는다.
 */
...
export type TutorialStep = {
  ...
  what: string;
  /**
   * 어려운 낱말과 그 뜻. **화면에 그 낱말이 실제로 보일 때만 단다** — 화면에 없는
   * 낱말을 설명하면 아이가 어디서 그 말을 찾아야 하는지 몰라 오히려 헷갈린다. 없으면
   * 말풍선이 자리·설명(`what`)만으로 끝난다.
   */
  term?: string;
  /** 그게 투자에서 무슨 뜻인지. 한 문장으로 끝낸다 — 접어 두었다가 눌러야 펼친다. */
  concept?: string;
  /**
   ...
   *   `다음` 이 켜진다).
   *
   * **자리를 옮기는 모든 장에 진입로가 있다** [2026-08-17 확정]. 예전에는 주문 단계
   * 사이를 비워 두고 아이가 직접 고르기를 기다렸다 — 대신 고르는 것이 원본 튜토리얼의
   * 문제였기 때문이다. 그런데 그러면 `다음` 이 아무 일도 안 하는 장이 넷이나 생겨
   * 안내가 거기서 끊겼다. 지금은 튜토리얼이 **보기 좋은 기본값**(1만원·첫 이유·첫
   * 계획)을 눌러 끝까지 데려가고, 아이는 설명을 읽은 뒤 그 자리에서 직접 바꾸면 된다.
   */
...
export const TUTORIAL_STEPS: readonly TutorialStep[] = [
  ...
  },
  {
    ...
    title: "오늘 내가 얼마나 벌었나",
    what: "여기 얼마나 벌었는지 내가 좋아하는 물건 개수로 쉽게 보여줘요. 아래 숫자로 정확한 금액도 볼 수 있어요.",
    term: "수익",
    concept: "처음보다 지금 가진 돈이 많으면 번 거고, 적으면 잃은 거예요.",
    enter: { path: "/" },
  },
  {
    id: "nav-trade",
    ...
    term: "모의투자",
    concept: "진짜 돈이 아니라 연습용 돈으로 해 보는 투자예요.",
  },
  {
    id: "explore-chips",
    screen: "explore",
  ...
  },
  {
    ...
    term: "주식과 주가",
    concept: "주식은 회사를 잘게 나눈 조각이고, 그 조각 값이 주가예요.",
  },
  {
    id: "detail-chart",
    screen: "stock",
    ...
    term: "차트",
    concept: "값이 오르내린 길을 선으로 이어서 그린 그림이에요.",
  ...
  },
  {
    ...
    term: "회사와 주식",
    concept: "주식을 산다는 건 그 회사의 작은 주인이 된다는 뜻이에요.",
  },
  {
    id: "detail-news",
    screen: "stock",
    ...
    term: "뉴스와 주가",
    concept: "회사에 좋은 일이 생기면 사려는 사람이 늘어서 값이 움직이곤 해요.",
  },
  {
    id: "detail-buy",
    screen: "stock",
  ...
  },
  {
    ...
    term: "주문",
    concept: "사겠다 또는 팔겠다고 내는 신청이 주문이에요.",
  ...
  },
  {
    ...
    term: "예약 주문",
    concept: "얼마가 되면 사겠다고 미리 적어 두는 주문이에요.",
  },
  {
    id: "buy-reason",
    screen: "order",
  ...
  },
  {
    ...
    term: "체결",
    concept: "주문이 실제 거래로 바뀌는 걸 체결이라고 해요.",
  ...
  },
  {
    ...
    term: "매도",
    concept: "주식을 파는 걸 매도라고 하고, 팔면 그만큼 돈이 지갑에 들어와요.",
 ...
 * 두 장이 같은 자리인지.
 *
 * 같은 자리면 `다음` 이 그 자리에서 바로 넘어간다. 다른 자리면 화면부터 옮겨야 하므로
 * `다음` 이 그 자리의 진입로(`enter`)를 실행해 데려간다.
 */

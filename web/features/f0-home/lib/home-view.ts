...
export type HomeView = {
  ...
  profitText: string;
  rateColor: string;
  /** 수익률 부호에 따른 황소 마스코트: 양수=엄지척, 보합(0)=걱정, 음수=울음. */
  moodImg: string;
  /** 계좌를 읽었는데 보유가 하나도 없을 때만 참. */
  noHoldings: boolean;
};

/** 수익률 부호별 홈 마스코트. 0은 보합으로 본다. */
export function moodImg(rate: number): string {
  if (rate > 0) return "/ui/assets/mascot-bull.png";

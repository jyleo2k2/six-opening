...
- **튜토리얼이 사고파는 회사는 `TUTORIAL_STOCK`(크래프톤) 하나로 고정한다.** 아이가 카드를 어디까지 밀어 놨든 같은 회사로 들어가야 뒤따르는 설명이 맞는다. 예전에는 보고 있던 카드를 대신 눌렀는데, `id="tut-explore-cards"`가 붙은 것은 카드를 감싼 슬라이드이고 `onClick`은 그 안쪽 카드에 있어 흐름이 카드에서 끊겼다. **자녀 계정의 시드 보유 종목이어야 한다** — 뒷부분이 파는 과정인데 매수 장의 주문은 장이 열려 있을 때만 즉시 체결돼서, 갖고 있지 않으면 장 밖에서 매도 화면이 막힌다.

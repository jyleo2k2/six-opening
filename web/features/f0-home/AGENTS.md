...
- **짚을 앵커가 화면에 실재하는지는 `tutorial-steps.test.ts`가 지킨다.** `features/f0-home`의 `.tsx`를 읽어 `id="tut-*"`를 모으고 장의 앵커가 전부 그 안에 있는지 본다. 예외 목록을 만들지 않는다 — 예전에 `tut-detail-news`가 화이트리스트로 비켜 가 있어 상세에서 `다음`이 조용히 아무 일도 안 했다.

...
- 지갑(`kw_proto_v1`)은 `lib/use-wallet.ts`, 서버 계좌(`/api/account`)는 `lib/use-account.ts`, 관심 종목(`/api/watchlist`)은 `lib/use-watchlist.ts`, 시세·유니버스는 `lib/use-universe.ts`, 아카이브의 성향·가족·반응은 `lib/use-archive-data.ts`로만 읽는다. **홈과 아카이브는 지갑이 아니라 서버를 본다** — 로그인한 사람의 실제 보유·성향이 원본이다.
- **저장소(`kw_proto_v1`)에 남는 것은 매수·매도 기록뿐이다.** 현금·보유·미체결 주문은 마운트마다 서버로 다시 세우므로 저장하지 않고, 관심 종목은 `/api/watchlist`가 원본이다. 새 값을 지갑에 얹기 전에 그것을 담을 서버 경로가 먼저 있어야 한다.

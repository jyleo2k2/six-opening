...
| `web/features/f0-home/lib/archive-feed.ts` | 카드 값 계산 — 서버가 주지 않은 값을 화면에서 추론하지 않는다. 깔 장수(`FEED_LIMIT`·`FEED_PER_MEMBER`)도 여기가 정한다 |
| `web/features/f0-home/lib/use-archive-data.ts` | `/api/family` 50건 페이지 조회·누적, 페이지별 거래 id 반응 조회, 좋아요 토글·코멘트 작성·본인 삭제 |
| `web/app/api/family/route.ts` | 가족 범위, 구성원 성향, 구성원별 수익률, **가족 자산 합계(`total`)**, 체결 50건 페이지. 체결가·수량은 가리지 않고 구성원 줄의 평가금액·원금·현금만 가리며, **보유가 없는 종목의 체결은 응답에서 뺀다** |
...
`useArchiveData()` 가 아카이브 진입 시 `/api/family?offset=0`으로 첫 50건을 호출하고, 응답의 체결 id로 댓글·좋아요를 이어서 조회한다. 사용자가 수익률 탭을 아래로 내려 끝에서 240px 안에 들어오고 **아직 카드가 여섯 장에 못 미칠 때만** `page.nextOffset`으로 다음 50건을 한 번 요청해 기존 목록 뒤에 붙인다. 화면에 까는 것은 여섯 장뿐이므로(§3.3) 이미 찼으면 더 읽어도 보이는 것이 늘지 않는다 — 그런데도 페이지가 필요한 이유는 아래 보유 종목 필터 때문에 한 페이지가 여섯을 못 채울 수 있어서다. 서버는 클라이언트가 넘긴 가족값을 받지 않고 세션 사용자의 `family_tag`로 범위를 정한다. 태그가 없으면 본인만 반환한다.
...
**`hasMore`는 보유 종목 필터를 걸기 _전_ 행 수로 센다.** 걸러진 수로 세면 한 페이지가 통째로 걸러졌을 때 `hasMore`가 거짓이 되어 그 뒤 기록이 영영 안 보인다. 그래서 `trades`가 비어 있는데 `hasMore`가 참인 페이지가 나올 수 있다 — 정상이다.
...
| 가격이나 밑값이 없는 행 | 서버가 `null`로 주므로 **`비공개`**, 손익 줄은 빈 문자열 |
| 이유 | `trade_reason`, 없으면 빈 상태 문구 |

카드는 `trade.price`를 표시하므로 현재 문구의 금액은 **총 거래금액이 아니라 주당 단가**이다.

**체결가·수량은 가족끼리 가리지 않는다**(2026-08-17 유저 확정). 서로의 기록을 견주어 보라고 만든 화면인데, 타인 값을 `null`로 지우면 자기 카드만 숫자가 있고 나머지는 전부 `비공개`라 견줄 것이 남지 않았다. 자산 규모는 여기가 아니라 `members`·`total`이 지킨다 — 구성원 줄에 평가금액·원금·현금을 싣지 않고 합계만 낸다. 체결 한 건의 단가·수량으로는 그 사람이 얼마를 굴리는지 알 수 없다.
...
2. **카드의 원 표시는 총 거래금액이 아니라 주당 체결가인데 라벨이 그 구분을 하지 않는다.** 매수·매도 양쪽 모두 해당한다. 2026-08-17 마스킹을 걷어내면서 이제 **남의 카드에서도** 보이므로 오해의 범위가 넓어졌다. 라벨을 주당 단가로 명시하거나 `price × quantity`를 찍으면 해소된다.

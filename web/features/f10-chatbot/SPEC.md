...
- **키웅이 아바타는 헤더가 아니라 답변 위 이름줄에 대화 상대 프로필로 선다.** 봇 메시지는 `아바타 + 이름표` 한 줄 → 말풍선 → 칩 순으로 쌓는다. 아바타는 플로팅 버튼과 같은 이미지(`floatingAvatar.src`)를 32px 원형으로 쓰고 봇 메시지마다 붙으므로, 대화가 길어져도 누가 말하는지가 스크롤 밖으로 밀려나지 않는다. 이름표는 `키웅AI`다.
- **아바타는 이름줄에 아래 절반만 걸친다.** 음수 위 여백(`-mt-4`, 16)으로 flex 항목의 바깥 높이를 글씨 높이(16)로 줄여 이름줄은 그대로 두고 동그라미만 위로 16 삐져나오게 한다. 목록의 위 여백(`pt-4`, 16)이 그만큼을 이미 비워 두므로 첫 답변에서도 잘리지 않는다. 말풍선과 그 아래 칩·화면 이동 버튼은 아바타 폭(32 + 간격 8 = 40)만큼 들여써 왼끝을 맞추고, 말풍선은 이름줄 아래로 6 내려 이름표에 붙지 않게 한다. 자녀 말풍선에는 프로필도 이름표도 두지 않는다.

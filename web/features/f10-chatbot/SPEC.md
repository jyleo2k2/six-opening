...
- `orderMethodConfusion`의 “직접 질문”은 고정 발화를 첫 메시지로 남긴 채 채팅창을 열고, 이어서 시장가 DAPIE 설명을 시작한다. DAPIE 선택지와 추천 질문을 통해 지정가 설명으로 이어진다.
- `dwell`·`lossRevisit`의 기존 **직접 질문 / 괜찮아** 반응 카드는 유지한다.
...
- `dwell`·`lossRevisit`에서 “괜찮아”를 누르면 현재 카드만 닫고 이후 재발동은 막지 않는다.
- 30분 무활동이면 새 세션으로 본다.

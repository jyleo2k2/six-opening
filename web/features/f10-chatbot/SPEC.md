...
- **호스트 기준 사각형은 `PhoneFrame` 안 `#kw-screen`(402×874)의 `getBoundingClientRect()` 실측 하나로 정한다.** `ConnectedPrototype`이 CSS로 배치된 실제 client rect를 읽어 F10·clip·두 번째 프레임·`TutorialOverlay`에 전달하므로 주소창·키보드·핀치 줌에서도 fixed 오버레이와 프레임이 같은 좌표계를 쓴다. 갱신은 `ResizeObserver`와 `visualViewport`의 `resize`·`scroll` 이벤트를 rAF로 합친다. F10 단독 데모에서 호스트 값이 없을 때만 `getPrototypeScreenRect`를 fallback으로 쓴다. 폰 치수 원본은 `PROTOTYPE_PHONE` 상수다.

...
- **키웅이 아바타는 헤더가 아니라 답변 말풍선 왼쪽에 대화 상대 프로필로 선다.** 플로팅 버튼과 같은 이미지(`floatingAvatar.src`)를 32px 원형으로 쓰고, 봇 메시지마다 붙으므로 대화가 길어져도 누가 말하는지가 스크롤 밖으로 밀려나지 않는다. **말풍선 묶음 기준 하단 정렬**이다 — 상단 정렬하면 칩이 길게 붙은 답변에서 얼굴만 위로 떨어져 다음 말풍선 것처럼 읽힌다. 봇 말풍선과 그 아래 칩·화면 이동 버튼은 아바타 폭(32 + 간격 8)만큼 오른쪽으로 밀린다. 자녀 말풍선에는 프로필을 두지 않는다.

...
- 모든 선제 도움은 브라우저 바깥 카드가 아니라 MTS 화면 안에서 플로팅 챗봇 버튼 위쪽에 여백을 두고, 아래 꼬리가 버튼을 향하는 한 줄 말풍선으로 표시한다. 말풍선과 플로팅 챗봇 버튼을 누르면 같은 고정 발화로 대화를 시작하며, 별도의 **직접 질문 / 괜찮아** 버튼은 두지 않는다. 고정 응답·추천 질문·DAPIE 선택지는 모두 해요체를 사용한다.

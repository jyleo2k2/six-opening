...
플로팅 버튼을 화면 하단 가운데의 **동그라미 X** 위에 놓으면 버튼이 삭제되지 않고 오른쪽 화면 경계에 반쯤 걸친 퍽으로 접힌다. 산식과 좌표는 `lib/floating-avatar.ts` 가 갖고 화면은 표시만 한다.
...
- **여는 순간 바로 전체가 나타나지 않는다.** 시트가 화면 아래에서 위로 슬라이드업하고, 다 올라온 시점(진입 트랜지션 종료)에 헤더의 `초기화` 버튼 바로 왼쪽 자리에 키웅이 아바타가 페이드인한다. 플로팅 버튼은 스크림이 이미 그 자리를 즉시 덮으므로 별도 퇴장 모션은 두지 않는다. 이미 열려 있는 상태에서 메시지를 보내 시트를 다시 부르는 호출은 이 모션을 재생하지 않는다 — 전송마다 시트가 오르내리면 산만하다.

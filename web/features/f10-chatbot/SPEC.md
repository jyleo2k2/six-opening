...
- 본문을 읽기 전에 사용자별 요청 수를 센다. 1분에 12건을 넘으면 `429`로 거절한다. 한 요청이 Luna를 최대 4회 부르므로(§5.3) 연타는 게이트 앞에서 끊는다. 거절된 요청은 카운터에 세지 않아 1분보다 오래 잠기지 않고, 화면은 실패를 “키웅이가 잠깐 낮잠 중이에요” 안내로 보여 준다.

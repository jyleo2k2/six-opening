...
- **아바타의 세로 자리는 레이아웃 값과 보이는 값을 나눠 정한다.** 음수 위 여백(`-mt-6`, 24)은 레이아웃 몫으로, 40 짜리 아바타의 flex 바깥 높이를 글씨 높이(16)까지 줄여 이름줄이 얼굴만큼 두꺼워지지 않게 한다 — 그래야 아래 말풍선이 밀려 내려가지 않는다. 상대 위치(`top-2`, 8)는 보이는 몫으로, 동그라미만 8 내려 말풍선 위끝에 살짝 걸치게 한다. 이름줄 위로 삐져나오는 양은 `40 - 16 - 8 = 16`이고 목록의 위 여백(`pt-4`, 16)이 그만큼을 이미 비워 두므로 첫 답변에서도 잘리지 않는다. 말풍선과 그 아래 칩·화면 이동 버튼은 아바타 폭(40 + 간격 8 = 48)만큼 들여써 이름표와 왼끝을 맞추고, 말풍선은 이름줄 아래로 6 내려 이름표에 붙지 않게 한다. 자녀 말풍선에는 프로필도 이름표도 두지 않는다.

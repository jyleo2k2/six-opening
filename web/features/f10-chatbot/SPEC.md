...
- **여는 순간 바로 전체가 나타나지 않는다.** 시트가 화면 아래에서 위로 1초 동안 슬라이드업하고, 헤더의 `초기화` 버튼 바로 왼쪽 자리에 있는 키웅이 아바타는 시트와 함께 올라온다. 플로팅 버튼은 스크림이 이미 그 자리를 덮으므로 별도 퇴장 모션은 두지 않는다. 이미 열려 있는 상태에서 메시지를 보내 시트를 다시 부르는 호출은 이 모션을 재생하지 않는다 — 전송마다 시트가 오르내리면 산만하다.

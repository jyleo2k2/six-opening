...
| 코드 | 조건 | 고정 발화 |
|---|---|---|
| `buyHesitation` | 같은 종목의 매수 5/5 최종 확인 화면에서 `주문 넣기!` 없이 좌측 상단 뒤로가기로 30분 세션 안에 2회 이탈. 다른 화면 이용은 사이에 허용하며 매수 체결 시 해당 종목 기록 초기화 | “살지 말지 고민돼?” |
...
- `dwell`·`lossRevisit`에서 “괜찮아”를 누르면 해당 신호는 세션 동안 뮤트한다.
- 30분 무활동이면 새 세션으로 본다.
...
- `buyHesitation`·`dwell`·`lossRevisit` 판정, 발화 간 3분·동일 신호 10분·세션 최대 2회·신호 뮤트·30분 초기화를 `shared/engine` 순수 함수로 구현했다.
...
| 5 | 실제 사용자 골든 패스 | 모바일 화면에서 플로팅 버튼→휴대폰 화면 80% 시트, 상단 20% 탭·하향 드래그 닫기와 임계값 미만 복귀, FAQ→`uiAction`, 같은 종목 매수 5/5 뒤로가기 2회→한 줄 선제 말풍선, 추천 유도→고정 거절을 브라우저 E2E로 고정한다. |
...
- 골든 패스 ③에서 같은 종목 매수 5/5 뒤로가기 첫 번째에는 발화하지 않고, 다른 화면을 거쳐 돌아온 두 번째 이탈에 채팅창을 열지 않은 채 “살지 말지 고민돼?” 말풍선이 표시된다. 말풍선과 플로팅 챗봇은 같은 맥락의 채팅창을 연다.

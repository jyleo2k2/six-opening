...
- 시트 헤더는 드래그 핸들과 우측 `초기화` 버튼만 표시한다. **버튼 줄과 구분선 사이에 아래 여백을 두지 않는다** — 버튼이 이미 자기 패딩(`py-2`)을 가지고 있어 여백을 더하면 선이 그만큼 내려앉고 헤더가 내용보다 두꺼워 보인다. 초기화는 시트를 닫지 않고, 대화·입력·진행 중인 DAPIE/종목/섹터 후속 상태를 비우며 전송 중인 요청은 취소한다.

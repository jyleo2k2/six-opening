...
| 코드 | 조건 | 고정 발화 |
|---|---|---|
| `buyHesitation` | 매수 5/5 최종 확인 화면에서 `주문 넣기!` 없이 좌측 상단 뒤로가기로 이탈한 행동이 30분 세션 안에 종목과 무관하게 3회 연이어 발생. 매수 체결 시 누적 이탈 기록 초기화 | “살지 말지 고민돼?” |
...
- 모든 선제 도움은 브라우저 바깥 카드가 아니라 MTS 화면 안의 한 줄 말풍선으로 표시한다. 말풍선과 플로팅 챗봇 버튼을 누르면 같은 고정 발화로 대화를 시작하며, 별도의 **직접 질문 / 괜찮아** 버튼은 두지 않는다.
...
- 골든 패스 ③에서 종목과 무관하게 매수 5/5 뒤로가기 두 번째까지는 발화하지 않고, 세 번째 연속 이탈에 채팅창을 열지 않은 채 “살지 말지 고민돼?” 말풍선이 표시된다. 말풍선과 플로팅 챗봇은 같은 맥락의 채팅창을 연다.

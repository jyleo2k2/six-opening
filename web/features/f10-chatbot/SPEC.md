...
- **기준 사각형은 `getPrototypeScreenRect`(`lib/bottom-sheet.ts`) 순수 계산 하나로 정한다.** 화면과 챗봇이 같은 함수를 쓰므로 배율이 갈릴 자리가 없다. 예전에는 iframe 안 `#kw-screen`(402×874)을 `getBoundingClientRect()` 로 재서 `screenRect` 로 넘겼는데, iframe 철거(PR #296)로 재는 대상이 사라졌고 순수 계산이 실측식과 항등임을 14개 뷰포트에서 확인했다. 폰 치수 원본은 `PROTOTYPE_PHONE` 상수다.

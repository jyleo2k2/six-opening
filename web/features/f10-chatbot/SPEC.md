...
### 3.5 지시어 후속 질문 되살리기 [신설 2026-08-14]
...
- §3.5의 재작성은 답변을 만들지 않는 정리 호출이므로 위 1회 제한과 별도다. 1단이 허용 목적을 판정했거나 직전 답변이 없으면 호출하지 않는다. `reasoning.effort`는 `none`, `max_output_tokens`는 200이다.

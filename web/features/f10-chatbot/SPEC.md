...
- **키웅이 아바타는 헤더가 아니라 답변 위 이름줄에 대화 상대 프로필로 선다.** 봇 메시지는 `아바타 + 이름표` 한 줄 → 말풍선 → 칩 순으로 쌓는다. 아바타는 플로팅 버튼과 같은 이미지(`floatingAvatar.src`)를 40px 원형으로 쓰고 봇 메시지마다 붙으므로, 대화가 길어져도 누가 말하는지가 스크롤 밖으로 밀려나지 않는다. 이름표는 `키웅AI`다.
- **아바타의 세로 자리는 레이아웃 값과 보이는 값을 나눠 정한다.** 음수 위 여백(`-mt-6`, 24)은 레이아웃 몫으로, 40 짜리 아바타의 flex 바깥 높이를 글씨 높이(16)까지 줄여 이름줄이 얼굴만큼 두꺼워지지 않게 한다 — 그래야 아래 말풍선이 밀려 내려가지 않는다. 상대 위치(`top-6`, 24)는 보이는 몫으로, 동그라미만 내린다. **맞추는 것은 동그라미의 가운데**다 — 이름표 아래끝(16)과 말풍선 위끝(16 + `mt-1.5` 6 = 22) 사이 틈에 중심이 오게 `16 + 24 - 20 = 20`으로 놓는다. 위끝이 이름줄 위끝(0)과 같아 삐져나오는 부분이 없으므로 첫 답변에서도 잘리지 않고, 아래 절반은 말풍선 왼쪽 여백(x 0~40, 말풍선은 48부터)에 걸쳐 글씨를 가리지 않는다. 말풍선과 그 아래 칩·화면 이동 버튼은 아바타 폭(40 + 간격 8 = 48)만큼 들여써 이름표와 왼끝을 맞추고, 말풍선은 이름줄 아래로 6 내려 이름표에 붙지 않게 한다. 자녀 말풍선에는 프로필도 이름표도 두지 않는다.

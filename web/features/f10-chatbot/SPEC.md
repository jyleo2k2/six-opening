...
- **키웅이 아바타는 헤더가 아니라 답변 말풍선 왼쪽에 대화 상대 프로필로 선다.** 플로팅 버튼과 같은 이미지(`floatingAvatar.src`)를 32px 원형으로 쓰고, 봇 메시지마다 붙으므로 대화가 길어져도 누가 말하는지가 스크롤 밖으로 밀려나지 않는다. 맞추는 기준선은 **키웅이가 말한 답변 말풍선의 아래끝**이다 — 아바타를 말풍선과 같은 행에 두고 그 행만 하단 정렬한다. 칩까지 포함한 묶음 바닥에 맞추면 선택지가 여러 줄인 답변에서 얼굴만 저 아래로 떨어져 다음 차례 발화의 프로필처럼 읽힌다. 말풍선 아래에 붙는 칩·화면 이동 버튼은 아바타 폭(32 + 간격 8 = 40)만큼 들여써 말풍선 왼끝에 맞춘다. 자녀 말풍선에는 프로필을 두지 않는다.

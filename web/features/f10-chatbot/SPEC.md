...
- [사실] DAPIE 연구의 대화형 설명 근거는 `docs/DAPIE_논문_추출.md`에 정리한다. 이 앱에서는 금융 개념·서비스 사용법·화면 맥락·승인 종목 사실·본인 기록·성향·아카이브·허용 LLM 답변·선제 도움 등 **허용된 모든 정보성 답변**에 적용한다.
- 한 번의 응답은 **피드백 → 한 번에 개념 하나 설명 → 질문**으로 구성한다. 질문은 유도(Guiding)·진단(Diagnosis)·선지식 확인(Eliciting) 중 하나이며 버튼 선택지와 직접 입력을 함께 허용한다.
- 기존 승인 스크립트 8건(`per`·`pbr`·`eps`·`market-order`·`limit-order`·`unrealized-profit`·`realized-profit`·`diversification`)은 내용별 진단 질문을 유지한다. 그 밖의 용어·FAQ·동적 조회·허용 LLM 답변은 공통 유도 질문으로 시작해, 아이가 “더 쉽게”를 고르면 작은 질문이나 직접 질문으로 조정한다.
...
| 2 | 51종목 사실 검수 | 현재 51개가 모두 `draft`다. 출처·확인일·사람 검수를 마친 `reviewed` 항목만 Tool이 응답하게 한다. |
...
- 모든 허용 정보성 답변이 피드백·설명·질문과 검증된 선택지를 제공하고, 안전 응답만 즉시 고정 안내로 우회한다.

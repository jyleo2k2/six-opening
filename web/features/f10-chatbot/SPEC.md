...
- 모든 화면 우하단에 키웅이 플로팅 버튼을 둔다. 아래 3.2로 접힌 동안에는 오른쪽 가장자리에 반쯤 걸친 퍽으로 표시한다.
...
### 3.2 삭제 타깃 — 드래그해서 가장자리에 접기

플로팅 버튼을 화면 하단 가운데의 **동그라미 X** 위에 놓으면 버튼이 삭제되지 않고 오른쪽 화면 경계에 반쯤 걸친 퍽으로 접힌다. 접힌 퍽은 주문 1단계의 `예약` 탭 중심 높이에 맞추고 불투명도는 80%로 표시한다. 산식과 좌표는 `lib/floating-avatar.ts` 가 갖고 화면은 표시만 한다.
...
- 놓으면 열려 있던 바텀시트도 함께 닫고 퍽의 세로 자리는 유지한다. `ConnectedPrototype` 안에서 F10 컴포넌트가 유지되므로 홈·종목·매수 2단계에서도 같은 자리에 남는다.
- 접힌 퍽을 한 번 탭하면 기본 우하단 자리로 돌아온다. 복귀 탭에서는 시트를 열지 않고, 다음 탭부터 일반 플로팅 버튼처럼 모바일 바텀시트 챗 UI를 연다.
- **여는 순간 바로 전체가 나타나지 않는다.** 시트가 화면 아래에서 위로 1초 동안 슬라이드업한다. 플로팅 버튼은 스크림이 이미 그 자리를 덮으므로 별도 퇴장 모션은 두지 않는다. 이미 열려 있는 상태에서 메시지를 보내 시트를 다시 부르는 호출은 이 모션을 재생하지 않는다 — 전송마다 시트가 오르내리면 산만하다.
- 바텀시트는 브라우저 전체가 아니라 렌더된 휴대폰 화면에 맞추고, 그 화면 높이의 80%를 차지한다.
- **호스트 기준 사각형은 `PhoneFrame` 안의 450×920 프레임 내부 좌표 하나로 정한다.** `ConnectedPrototype`은 `PhoneFrame`을 하나만 만들고 F10·clip·`TutorialOverlay`를 그 프레임의 `.phone-stage__overlay` 안에 렌더링한다. 이 레이어에는 `PHONE_SCREEN_RECT`를 전달하고, 프레임 이미지(z-index:5)가 같은 stacking context에서 마지막에 그려진다. 따라서 바깥 fixed 오버레이·두 번째 프레임·viewport 보정값은 없다. F10 단독 데모에서 호스트 값이 없을 때만 `getPrototypeScreenRect`를 fallback으로 쓴다. 폰 치수 원본은 `PROTOTYPE_PHONE` 상수다.
...
- 시트 헤더는 드래그 핸들과 우측 `초기화` 버튼만 표시한다. **구분선은 버튼 줄에 붙이고 글씨는 선 위 빈 자리의 가운데에 놓는다** — 여백을 더하면 선이 버튼 패딩만큼 내려앉아 헤더가 내용보다 두꺼워 보이고, 버튼의 `py-2` 만 믿으면 위쪽 드래그 핸들 줄이 남긴 11 이 더 얹혀 글씨가 선에 붙어 보인다. 줄을 6 올리고(`-mt-1.5`) 선을 같은 6 내려(`pb-1.5`) 상쇄시키면, 헤더 높이와 선 위치는 그대로 둔 채 글씨만 가운데로 올라온다. 초기화는 시트를 닫지 않고, 대화·입력·진행 중인 DAPIE/종목/섹터 후속 상태를 비우며 전송 중인 요청은 취소한다.
...
- **키웅이 아바타는 헤더가 아니라 답변 위 이름줄에 대화 상대 프로필로 선다.** 봇 메시지는 `아바타 + 이름표` 한 줄 → 말풍선 → 칩 순으로 쌓는다. 아바타는 플로팅 버튼과 같은 이미지(`floatingAvatar.src`)를 50px 원형으로 쓰고 봇 메시지마다 붙으므로, 대화가 길어져도 누가 말하는지가 스크롤 밖으로 밀려나지 않는다. 이름표는 `키웅AI`다.
- **아바타의 세로 자리는 레이아웃 값과 보이는 값을 나눠 정한다.** 음수 위 여백은 레이아웃 몫으로, 아바타의 flex 바깥 높이를 이름표 줄높이(16)까지 줄여 이름줄이 얼굴만큼 두꺼워지지 않게 한다 — 그래야 아래 말풍선이 밀려 내려가지 않는다. 상대 위치(`top`)는 보이는 몫으로 동그라미만 내린다. **맞추는 것은 동그라미의 가운데**이며, 이름표 아래끝(16)과 말풍선 위끝(16 + `mt-1.5` 6 = 22) 사이 틈의 가운데 19 에 중심을 놓는다.
- **지름을 바꾸면 따라오는 값은 산식으로 정한다.** 지름 `D`에 대해 위 여백은 `16 - D`, `top`은 `D / 2 + 3`, 말풍선·칩 들여쓰기는 `D + 8`(아바타와 이름표 사이 `gap-2`)이다. 이름줄 위로 삐져나오는 양은 `D - 16 - top`이고 목록의 위 여백(`pt-4`, 16) 안에 들어와야 잘리지 않는다. 지금은 `D = 50`이라 `-mt-[34px]` · `top-7`(28) · 들여쓰기 58 · 삐져나옴 6 이다. 말풍선과 그 아래 칩·화면 이동 버튼은 그 들여쓰기로 이름표와 왼끝을 맞추고, 말풍선은 이름줄 아래로 6 내려 이름표에 붙지 않게 한다. 아바타의 아래 절반은 말풍선 왼쪽 여백(x 0~D, 말풍선은 D+8부터)에 걸치므로 글씨를 가리지 않는다. 자녀 말풍선에는 프로필도 이름표도 두지 않는다.
...
- **확인 질문(`turn.prompt`)은 답변과 같은 모양의 두 번째 말풍선으로 세운다.** 그 문장은 §3.3.2 에서 🤖 키웅이 발화이므로 봇의 채움(`bg-bg`)을 그대로 받아야 한다. 칩과 같은 flex 컨테이너에 채움 없는 회색 글씨로 두면, 시트가 흰 바탕이라 문장이 말풍선을 떠나 칩 쪽에 붙어 **버튼 그룹의 제목표**로 읽힌다. 그러면 한 응답의 3부 구성(피드백 → 설명 → 질문, §3.4)이 시각적으로 2:1 로 쪼개져 대화가 아니라 "설명문 + 설문 폼"이 된다. DAPIE·종목 탐색·섹터 탐색 세 경로가 같은 조각을 쓴다.
- **질문 말풍선에 굵기나 라벨을 더하지 않는다.** 이 시트에서 `font-medium` 은 칩이 쓰는 "누를 수 있다"는 신호라, 읽기만 하는 문장에 주면 질문이 칩과 같은 무게가 되어 눌리는 것처럼 보인다. `질문 :` 같은 접두도 두지 않는다 — 말풍선이 나뉘는 것·물음표·바로 아래 붙는 선택지가 이미 질문임을 알리고, 라벨은 §3.4 가 막아 둔 학습지 말투를 들여온다.
...
- 이 3문장은 **모든 답변 경로의 공통 상한**이다. DAPIE 설명 스크립트는 그보다 좁은 문장 예산을 따로 지킨다(§3.4.4).
...
- **없는 화면은 승인 사전에 “없다”를 말하는 항목으로 등록한다.** 어디에도 등록하지 않으면 어느 경로에도 안 걸려 기본 범위 안내(“저는 …도와주는 챗봇이에요”)로 끝나고, 아이는 왜 안 되는지 모른 채 화면에서 그것을 계속 찾는다. 현재 등록된 것은 호가창(`orderbook-unsupported`) 하나이며, 실제 MTS 에는 있고 이 모의투자에는 없는 화면을 발견할 때마다 같은 방식으로 늘린다.
- 미지원 안내의 트리거는 **가려질 낱말까지 적는다.** `호가` 한 낱말만 두면 트리거 길이가 `매수`·`매도`와 같아 용어 사전이 이기고 “매수는 주식을 사는 거래예요”가 나간다.
...
- **피드백은 문장이 아니라 머리말이다**(§3.4.4). Feedback 서브턴은 유지하되 설명 본문의 문장 예산을 먹지 않도록 짧은 한 마디로 둔다.
- T1 용어 36건(§9.1)과 2026-08-14 화면 용어 27건은 모두 내용별 진단·오답 조정·예시 복귀 스크립트를 가진다. 선차트·캔들차트·분봉·일봉·주봉은 독립된 용어와 독립 질문으로 처리한다. 동적 조회·허용 LLM 답변은 기존 공통 유도 질문을 유지한다.
...
- `detail`은 `brief`를 다시 말하지 않는다. 퀴즈를 맞힌 아이에게 방금 읽은 문장을 되풀이하면 정답의 보상이 반복이 된다 — `detail`은 `brief`에 **덧붙는 새 내용**이어야 한다(§3.4.4).
...
- 되묻기(`reaskExplain`)에는 마지막 선택지로 `{ id: "ask", label: "다른 걸 물어볼래요" }`를 붙여 버튼으로도 현재 설명을 빠져나갈 수 있게 한다. 첫 질문에는 붙이지 않으며, 이미 `ask` 선택지가 있는 단계는 그 선택지를 재사용한다.
...
- 절차 답변은 **실제 화면에 있는 것만** 가리킨다. 없는 버튼·화면을 지어내면 아이가 찾다가 못 찾는다. 이 서비스에 없는 화면은 절차 항목이 아니라 §3.3.1 의 미지원 안내 항목으로 답한다.
- §7 선제 발화가 내미는 추천 질문은 그 형태로 답할 지식이 있어야 한다. `proactive-followup.test.ts`가 형태 계약을, `routing.test.ts`가 스크립트 보유 용어 전수를 검사한다.

#### 3.4.4 설명 문장 예산 [신설 2026-08-16]

§6 T5 출력 게이트의 **최대 3문장은 상한이고, 이 예산은 평균을 정한다.** 상한만 두었더니 초과 0건인 채로 평균이 상한에 붙어(brief 2.73·detail 2.98·adjust 2.95문장) 아이가 매 턴 세 문장을 읽었다. 상한을 내리면 term 9종·업종·meta·rule 고정 응답(각 최대 3문장)까지 함께 막히므로 **게이트는 3문장 그대로 두고 승인 데이터에 예산을 건다.**

| 필드 | 예산 | 이유 |
|---|---|---|
| `brief` | **1문장** | 첫 답은 정의 하나면 된다. 단서·예외는 `detail`로 미룬다 |
| `detail` | **1문장** | `brief`에 덧붙는 새 내용. 되풀이 금지 |
| `adjust.explanation` | 2문장 이내 | 정답 근거를 직접 담아야 하므로 기존 계약 유지(§3.4) |
| `example` | 2문장 이내 | 비유는 상황과 대응을 함께 말해야 성립한다 |

- 예산은 `chatbot-knowledge.test.ts`의 정적 데이터 테스트가 강제한다. **`shared/llm/filter.ts`의 게이트를 조이지 않는다** — 그 게이트는 DAPIE 전용이 아니라 모든 답변 경로의 공통 상한이다.
- 조립된 말풍선은 `머리말 + 본문`이다. 피드백을 머리말로 강등했으므로(§3.4) 1턴은 `머리말 + brief 1문장`, 정답 뒤 2턴은 `머리말 + detail 1문장`이 된다.
- `brief`·`detail` 1문장은 **직접 쓴 용어 스크립트(`kind: "glossary"`) 전체**에 적용한다. 카테고리로 나누지 않는다.
- 1문장으로 줄일 때 **진단 질문의 답 단서를 그 문장 안에 남긴다.** 예전 2문장 구조는 "정의 + 단서"였고 단서 쪽이 퀴즈의 답이었다. 정의만 남기면 아이가 방금 읽은 문장으로는 답할 수 없다.
- 안전을 나르는 문장은 줄일 때도 남긴다 — “앞으로의 결과를 알려주지는 않아요”, “미래를 보장하지는 않아요”, “파는 때를 정해 줄 수 없어요”. 길이를 줄이려고 이 말을 빼면 F10 회귀의 차단 분포가 아니라 아이가 읽는 경고가 사라진다.
- **`screenTermScript` 가 만드는 화면 용어는 1문장 예산에서 제외한다.** 그 `brief` 는 FAQ `answer` 를 그대로 쓴 것이라 단답을 겸하는데, 둘째 문장이 “이미 체결된 주문은 취소할 수 없어요” 처럼 실제 정보를 나른다. 깎으면 길이 대신 정보를 잃는다.
- **`detail` 과 `example` 은 용어마다 따로 쓰고 공용 문장을 돌려쓰지 않는다.** 화면 용어 22종이 `screenTermScript` 에서 “화면에 이미 있는 값을 가리키는 말이라…” 한 문장을 함께 받던 자리다. `detail` 은 **정답 경로**라 맞힌 아이가 거의 모두 지나가는데 용어가 무엇이든 같은 말이 나와, 맞힌 보상이 남의 이야기가 됐다. 모의투자·시즌·주문 잠금·어린이 뉴스·성향처럼 “화면의 값”이 아닌 용어에는 사실도 어긋났다. `SCREEN_TERM_QUIZZES` 가 두 필드를 필수로 받아 조립 단계에서 빠뜨릴 수 없고, 정적 데이터 테스트가 겹침을 막는다.
- **`example` 은 조정 설명에서 쓴 비유를 되풀이하지 않는다.** `example` 까지 온 아이는 두 번 틀린 아이다. 방금 실패한 그림을 그대로 다시 내밀면 새로 쥘 것이 없으므로 다른 각도의 비유를 준다.

#### 3.4.5 조정 질문은 틀린 그 개념으로 돌아온다 [신설 2026-08-17]

조정 설명이 곁가지 개념을 풀어 주더라도 **질문은 아이가 틀린 개념을 다시 물어야 한다.** `term:pbr` 의 조정이 “재산”의 뜻만 묻고 PBR 로 돌아오지 않아, PBR 을 모르는 채로도 정답을 고를 수 있었다. §3.4 의 “원래 흐름으로 돌아간다”는 다음 단계로 넘어가는 것만이 아니라 **묻는 대상이 제자리로 오는 것**을 함께 뜻한다.

조정 선택지는 질문이 묻는 형태와 맞춘다. 명사를 물으면 선택지도 명사여야 한다 — `term:per` 의 조정이 “주가를 무엇과 견줄까요?” 에 “들어가요 / 들어가지 않아요” 를 내밀어 아이가 답할 수 없었다(질문만 새로 쓰고 선택지를 옛 예/아니오로 둔 흔적). 정적 데이터 테스트가 이 어긋남을 막는다.
...
- 진행 중인 설명에서 `그만`·`됐어`·`안 할래`·`다른거`·`딴거`·`배당 궁금해` 같은 이탈 표현은 별도 사전을 정규화한 뒤 **전체 입력이 정확히 일치할 때만** 일반 라우팅으로 넘긴다. 부분 일치로 이탈을 추측하지 않는다.
...
- 화면·종목이 바뀌면 화면 컴포넌트가 `ConnectedPrototype`에 채팅 맥락을 올린다. 호스트는 런타임 스키마를 통과한 값만 챗봇 요청에 사용한다.
...
- 클라이언트는 HTTP 본문 종료만으로 성공 처리하지 않는다. 완전한 `done` 이벤트, 빈 SSE 잔여 버퍼, 비어 있지 않은 `text`를 모두 확인한 뒤에만 답변을 저장한다. `done` 없이 끊기거나 이벤트가 반쪽이면 부분 답변을 버리고 재시도 가능한 연결 실패로 표시한다.
...
승인 사전은 어떤 크기여도 완전해지지 않는다. 사전에 없는 낱말은 금융 개념 판정 자체가 서지 않아 `outOfScope`로 떨어지고 `fallback`의 용어 분류에도 닿지 못했다 — **§1.1이 첫 번째 수행 기능으로 적은 금융 개념 설명이 사전 크기만큼만 동작했다.** held-out 실측(`records/f10-child-sim/LABELS.md`)에서 `상장폐지가 뭔데?`·`코스피랑 코스닥은 왜 나눠?`가 "그 범위에서 물어봐 주세요"로 끝났다.

- 라우팅이 **허용 목적 미판정 범위 안내**로 끝나고 응답이 고정 텍스트일 때만 실행한다.
- `fallback`과 같은 **9종 용어 분류기**를 부른다. 분류에 성공하면 그 9개의 승인 고정 문장을 쓰고, `none`이면 원래의 범위 밖 안내를 그대로 둔다.
- **자유 생성을 열지 않는다.** 이 분기가 만들 수 있는 결과는 승인 텍스트뿐이라 안전 하한이 내려가지 않는다. 모델은 분류기로만 쓴다.
- `fallback` 쪽 분류와 배타적이라 한 요청의 LLM 호출 상한 4회는 그대로다. 지연 예산은 4초(`termClassifyTimeoutMs`)이며 실패·시간 초과는 범위 밖 안내를 유지한다.
- 이 구제는 사전 확장을 대신하지 않는다. 승인 문장이 있는 쪽이 항상 낫다 — 고정 응답은 T5b를 타지 않는다.
...
| 순위 비교·자기비하 | “나만 못하는 것 같아”, **“친구는 벌써 10% 벌었대”** | 성향은 성적이 아니라는 설명·본인 기록 |
| **보호자 몰래 거래** [신설] | “엄마 몰래 살 수 있어?”, “안 들키게 팔고 싶어” | 거래 기록이 가족에게 보이는 이유·매수 방법 |
| 뉴스·주문 불안·반복 확인 | “계속 확인하게 돼”, “불안해서 못 누르겠어”, **“다 잃었어 어떡해”** | 변동성 개념·본인 기록 |
...
- **손실을 이미 봤다는 말에는 "이 돈이 진짜 사라졌나"를 먼저 답한다.** 이 리그의 돈은 연습용 가상 현금이고 실제 돈이 줄어든 것이 아니다. 그 사실을 빼고 불안만 다독이면 아이는 계속 그 자리에 있다. 판정은 **과거 완료형만** 본다 — `마이너스면 진짜 돈 잃은 거야?`는 평가손익과 실현손익을 가르는 개념 질문이지 정서 지원 자리가 아니다.
- **보호자 몰래 사고파는 방법은 알려주지 않는다.** 대신 이 리그의 거래 기록이 왜 가족에게 보이는지 말한다 — 가상의 돈으로 함께 연습하는 곳이라 숨길 필요가 없다는 것이 답이다. `몰래`는 영상·SNS 열람(§6.1.4)에도 걸리지만 매매 요구가 있으면 이쪽이 먼저다.
- **견주는 말에 매수 의사가 붙으면 추천 차단이 먼저다**(§6.1.5). `친구가 20퍼 벌었다는데 나도 따라 사면 늦어?`는 위로가 아니라 매수 판단 요구다.
...
**선제 신호가 떠도 접힌 퍽을 자동으로 펼치지 않는다.** 접힌 동안에는 말풍선도 표시하지 않고, 사용자가 퍽을 한 번 탭해 기본 자리로 돌린 뒤에야 같은 중심 좌표에 말풍선을 붙인다. 버튼을 접은 것과 “먼저 말 걸지 마”는 별개이며, 후자는 §7.1의 선제 도움 끄기로 따로 다룬다.
...
**고정 발화는 아이의 상태를 단정하지 않는다.** 신호가 아는 것은 행동뿐이고 마음은 모른다. 감정을 단정하는 문구(“후회되나요?” 류)는 아이가 느끼지도 않은 감정을 나무라는 것처럼 읽힌다. 관찰한 행동에만 붙는 문장을 쓴다.
...
**플로팅 버튼을 삭제 타깃에 버리는 것(§3.2)은 “챗봇 안 쓸래”이지 “먼저 말 걸지 마”가 아니다.** 버튼은 사라지지 않고 가장자리에 접힌 채 남으며, 한 번 탭하면 기본 자리로 돌아온다. 선제 도움을 끄고 싶을 때는 접힘과 섞지 않고 이 절의 별도 상태를 사용한다.
...
| **아니요** 누적 3회 (`PROACTIVE_OFF_DECLINES`) | 선제 도움 **전체**를 끈다 | **아이가 직접 켜거나 앱을 다시 켤 때까지.** 30분 세션이 갈려도 유지되지만 새로고침·재로그인에는 풀린다(§7.2) |
...
### 7.2 끄기의 수명은 한 번의 앱 사용이다 [개정 2026-08-18]

**전체 끄기는 저장하지 않는다.** 앱을 다시 켜거나 다시 로그인하면 선제 도움은 켜진 채로 시작한다. 로그아웃이 `window.location.href = "/"` 로 페이지를 새로 띄우므로 재로그인도 같은 자리에서 풀린다.

그전에는 `chat-behavior-store` 가 상태를 통째로 `localStorage`(`kiwoom-chat-behavior-unlimited-v1`)에 저장해 **`off` 가 브라우저에 영구히 남았다.** 서버를 다시 띄워도, 브라우저를 껐다 켜도, 다른 계정으로 로그인해도 그대로였다. 되살릴 스위치는 대화창 머리에 **꺼져 있을 때만** 보이는 버튼 하나뿐인데, 그 버튼을 찾으려면 이미 꺼졌다는 사실을 알아야 한다.

수명을 줄인 이유는 **끄기를 부르는 힘이 3회 거절보다 세기 때문**이다. `buyHesitation` 을 뺀 말풍선은 스스로 사라지지 않으므로(§7 — 8초 타이머는 `buyHesitation` 전용), 치우는 유일한 방법이 `아니요` 다. 화면을 정리하려고 누른 버튼 세 번이 기능을 영구히 없애면 §7.1 이 맞추려던 말의 세기가 다시 어긋난다.

- 저장에서 빼는 것은 `proactiveMute` 하나다. **행동 이벤트와 30분 세션은 계속 저장한다** — 새로고침으로 이탈 기록이 사라지면 신호가 조건을 채울 수 없다.
- 이미 꺼 둔 브라우저에는 v1 저장본이 남아 있다. 되읽기가 얕은 병합이라 그대로 두면 이 변경 뒤에도 꺼진 채로 뜨므로, `version: 2` 의 `migrate` 가 남은 `proactiveMute` 를 떨어뜨린다.
- `chat-behavior-store.test.ts` 가 저장본에 `proactiveMute` 가 없다는 것과 v1 저장본이 끄기를 되살리지 못한다는 것을 함께 지킨다.
- **끄기를 다시 영구화하려면 서버에 자리를 먼저 만든다.** 브라우저에 남기면 계정이 아니라 기기에 붙어, 아이가 끈 것을 부모 계정이 물려받는다.
...
### 9.1 금융 사전은 두 티어다 [신설 2026-08-16]

| 티어 | 판별 | 대상 | 개수 |
|---|---|---|---:|
| **T1 — DAPIE 용어** | `kind: "glossary"` (스크립트 필수) | 주문·수익·지표처럼 **행동에 직접 붙는 말**. 대화형 확인이 값을 한다 | 36 |
| **T2 — 즉답 정의** | `kind: "faq"` (스크립트 없음) | 화면과 뉴스에 나오지만 아이가 **그 낱말로 결정을 내리지는 않는 말** | 126 |

- `kind: "glossary"`인 항목은 `explainScript.id === term:{id}`를 **반드시** 갖는다(`chatbot-knowledge.test.ts`가 검사한다). T2는 그래서 `faq`다.
- T2는 `termLabel`·`category`를 두지 않는다. "비슷한 용어" 추천 카드는 DAPIE로 이어지는 자리인데 T2에는 이어질 스크립트가 없다.
- **넣을 말을 고르는 기준은 하나다 — 앱 화면이나 어린이 뉴스에 실제로 나오는 낱말.** 자의적이지 않고, 화면·뉴스 텍스트로 검증할 수 있고, 멈출 지점이 있다. 뉴스 쪽 근거는 `f2-trade/prototypes/child-news-role-pipeline/term-frequency.ts`가 집계한다.
- 뉴스 쪽 근거는 **발행분 전수 집계**로 본다. 2026-08-16 실측(발행 101건·용어 230종)에서 사전에 없던 160종 가운데 2회 이상 반복되거나 기업 활동을 읽는 데 계속 걸리는 19종을 3차로 넣었다(계약·공장·지분·공시·인수·매각·이사회·지주사·법인·소각·연결 기준·수익성·성장률·반등·공급망·연료비·조달·최고경영자·사모펀드). 표기가 흔들리는 말(`전년 동기 대비`)은 항목을 늘리지 않고 트리거만 붙인다.
- **기사 고유명사와 생활·연예 낱말은 넣지 않는다**(세포라·편의점·월드투어). 그 낱말의 답은 그 기사에 붙은 쉬운 말 풀이지, 문맥 없이 읽혀야 하는 사전 문장이 아니다.
- **두 글자 트리거는 낱말 첫머리에서만 걸린다.** 조회는 공백을 지운 문자열로 하므로 경계가 사라져 `공[시가] 뭐야?`가 `시가`로, `수주잔[고가]`가 `고가`로 답하던 자리다(실측 2026-08-16). `findChatbotKnowledge(query, rawQuery)`의 둘째 인자로 원문을 함께 넘겨 앞 글자가 한글이면 버린다 — 라우터가 이 인자를 빼면 사고가 되살아나고 `chatbot-knowledge.test.ts`가 실패한다. 세 글자 이상은 우연히 겹칠 확률이 낮고, 겹쳐도 길이 정렬이 긴 쪽을 고른다.

### 9.2 다루지 않는 상품은 뜻과 경계를 함께 말한다 [신설 2026-08-16]

이 서비스는 51종 국내 주식을 가상 현금 안에서만 사고파는 모의투자다. 공매도·선물·파생상품·신용거래·공모주는 다루지 않고, 수수료와 세금은 계산하지 않는다(§4).

아이는 뉴스와 영상에서 그 말을 듣고 와서 묻는다. **모른 척하면 다른 데서 답을 찾고, 뜻만 알려 주면 여기서도 되는 줄 안다.** 그래서 두 가지를 한 답에 둔다.

1. 그 말이 무엇인지 — 10~13세 눈높이로 한 문장.
2. **여기서는 하지 않는다** — "이 모의투자에서는 하지 않아요" · "주식만 다뤄요" · "가진 가상 현금으로만 주문해요" · "계산하지 않아요".

**하는 방법은 설명하지 않는다.** 뜻과 경계까지가 이 사전의 범위다. 경계 문장이 빠지면 `chatbot-knowledge.test.ts`가 실패한다.

없는 화면의 용어는 아예 넣지 않는다 — `호가`는 `orderbook-unsupported`가 "이 서비스는 모의투자라서 호가창은 제공하지 않아요"로 답한다. 정의를 덧붙이면 있는 줄 안다.
...
- 승인 지식 항목 **162개**(DAPIE 용어 36 · 즉답 126)를 우선 검색하며, 가장 긴 동의어 일치 항목을 선택한다. 두 글자 동의어는 낱말 첫머리에서만 잡는다. 사전은 **두 티어**다(§9.1).
...
- 본인 거래 기록과 보유 현황 Tool은 `app/api/chat/personal-data.ts`가 `transactions`·`holdings`를 조회해 실제 값으로 답한다. 조회 대상은 요청 쿠키의 세션 사용자로만 정하고, 승인 목록에 없는 거래 이유 코드는 라벨을 만들지 않는다. 거래는 201번째 행까지 확인해 200건 이하일 때만 전체 개수를 말하고, 더 있으면 **“투자 기록이 200개보다 많아요. 최근 200개를 확인했어요”**라고 조회 상한임을 밝힌다.
...
- **아카이브로 보내는 `uiAction`은 탭을 반드시 적는다.** 거래 하나하나와 그때 고른 이유는 수익률 탭 피드(F11)에 있으므로 거래 기록 질문은 `archiveTab: "return"`, 주차별 기록은 성향 탭 위에 열리는 `archiveTab: "report"` + `archiveOverlay: "cards"` 다. 탭을 비우면 `ArchiveScreen` 이 말없이 성향 탭으로 떨어뜨려 엉뚱한 화면이 열린다. 같은 이유로 화면에 없는 탭 이름(`compare`·`season`)은 계약에서 뺐다.
...
- SSE `action` 이벤트와 허용 화면 `uiAction`을 답변 아래 버튼으로 구현했다. `ConnectedPrototype`이 허용 필드를 React 주소로 바꾸며, 주문 단계는 앞 단계 초안이 준비된 경우에만 건너뛴다. 모델 URL은 받지 않고 사용자가 버튼을 눌러야 이동한다.
- `buyHesitation`·`dwell` 판정과 30분 행동 누적 세션 초기화를 `shared/engine` 순수 함수로 구현했다. 시간·동일 신호·세션 횟수·뮤트 기반 재발동 제한은 두지 않는다.
- 화면 진입·가시 탭 체류·매수 최종 확인 이탈을 공용 zustand 이벤트 저장소에 연결했다.
- 대화 원문 없이 요청 ID·목적·경로·Tool·필터·실패 결과만 구조화 로그로 남긴다.
- T1 용어(`kind: "glossary"`) 36건은 모두 EPS와 같은 내용별 진단·오답 조정·예시 복귀 DAPIE 전이를 사용한다. T2 즉답 정의는 스크립트 없이 승인 문장만 낸다(§9.1).
...
7. `PhoneFrame`(`features/f0-home`)의 휴대폰 외곽·화면 치수를 바꾸면 `lib/bottom-sheet.ts`의 `PROTOTYPE_PHONE` 기준값과 테스트를 같은 변경에서 갱신한다.
...
- T1 용어 36건이 내용별 피드백·설명·질문과 검증된 선택지를 제공하고, T2 즉답 정의·서비스 사용법 FAQ·화면 맥락형 사용 안내는 DAPIE 선택지 없이 승인 답변과 허용 화면 액션을 바로 제공한다(§9.1). 다루지 않는 상품 항목은 뜻과 서비스 경계를 한 답에 담는다(§9.2).

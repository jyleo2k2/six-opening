...
- 버튼 중심이 `DISMISS_TARGET_SNAP_RADIUS_PX = 45`(화면 배율 반영) 안에 들어오면 타깃이 64px → 78px 로 커지고 마젠타로 채워지며, 버튼은 0.72 배로 줄고 흐려진다. 판정은 사각형이 아니라 **원**이다.
- 타깃은 버튼보다 위에 그린다. 버튼이 X 를 가리지 않고 빨려 들어가 보인다.
- 문구는 타깃 **위쪽**에 둔다(아래는 탭바). 평소 "여기에 놓으면 사라져요", 반경 안 "놓으면 사라져요".

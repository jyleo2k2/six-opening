...
assert.deepEqual(
  ...
  [
    ...
    "105560:finance", "055550:finance", "086790:finance", "316140:finance",
...
for (const stock of STOCKS) {
  ...
  // 더블유게임즈는 소셜 카지노 게임 회사다. 사실은 맞지만 아동 적합성 판단이
  // 남아 draft로 둔다. 사람이 판단한 뒤 reviewed로 올린다.
  assert.equal(
    education.status,
    stock.id === "KRX:192080" ? "draft" : "reviewed",
    `${stock.name} 검수 상태가 예상과 달라`,
  );

...
- 오케스트레이터만 수정한다.
- 현재 사용자 화면 라우트는 `/`와 iframe 차트용 `/tradingview-chart`다. `/`의 `page.tsx`는 서버에서 `kw_uid` 세션 쿠키(`api/supabase.ts`의 `findProfileById`)를 확인해, 로그인 상태가 아니면 `LoginGate`(스플래시+로그인, 가입은 준비 중 화면)를, 로그인 상태면 `features/f0-home/ConnectedPrototype`을 렌더링한다. 홈·탐색·상세·주문·계좌·랭킹·아카이브 화면 전환은 로그인 이후 `public/ui/app.html` 내부 상태가 소유한다. `/stocks`·`/trade/[symbol]`·`/archive` 라우트가 있다고 가정하지 않는다.

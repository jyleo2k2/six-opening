...
- `page.tsx`는 서버에서 `kw_uid` 세션 쿠키(`api/supabase.ts`의 `findProfileById`)를 확인해, 로그인 상태가 아니면 `LoginGate`(스플래시+로그인, 가입은 준비 중 화면)를 렌더링한다. 로그인 뒤에는 **옮겨 온 화면(`/ranking`)은 React가 직접**, 나머지 화면 전환은 `ConnectedPrototype`이 띄우는 `public/ui/app.html` 내부 상태가 소유한다.

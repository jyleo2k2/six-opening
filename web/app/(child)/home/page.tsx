...
import { pnlClass, rate, won } from '@/lib/format';
...
export default async function HomePage() {
  ...
  const holdingsValue = account.holdings.reduce((sum, h) => sum + h.price * h.qty, 0);
  const total = account.cash + holdingsValue;
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <section className="rounded-2xl bg-neutral-900 p-5 text-white">
        ...
        <p className="mt-3 text-xs text-neutral-400">현금 {won(account.cash)}</p>
      </section>
          ...
          >
            <div>
              ...
              <p className="text-xs text-neutral-500">
                {h.qty}주 · 평균 {won(h.avgCost)}
              </p>
            </div>
            <div className="text-right">
              <p className="font-semibold">{won(h.price * h.qty)}</p>

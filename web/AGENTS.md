...
- Use fixtures first. LLM access is server-only through `shared/llm` using `openai` and the `.env` `OPENAI_MODEL`; external data passes only through `app/api/quote`, never direct client calls or `NEXT_PUBLIC_*` keys.

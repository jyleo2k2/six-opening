...
키움증권 KDA 프로젝트. 부모·자녀 화면 + 2~8인 히스토리 투자 시뮬 게임(과거 10년의 실제 사건 속에서
뉴스·찌라시·이벤트로 겨루는 멀티플레이) + 투자제안서(제안→승인→체결)를
...
| 경로 | 내용 |
|---|---|
| `docs/기획서.md` | **기획서 (2026-08-07, 히스토리 투자 시뮬). 게임 룰 결정의 근거는 여기다.** |
| `docs/구현계획.md` | **구현계획 — 마일스톤·트랙 경계** |
| `docs/기술스택.md` | 기술 스택 결정 기록 |
| `game/` | 룰 엔진(SSOT)·종목/이벤트/뉴스 데이터·Colyseus 서버·밸런스 시뮬 |
| `web/` | Next.js 자녀 셸 + 부모 뷰 |
| `content/` | 도감·뉴스 MDX (사람 검수 후 커밋) |
| `pipelines/` | Python(uv) 배치 — 뉴스 재작성 |

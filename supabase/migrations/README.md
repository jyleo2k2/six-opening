...
| `20260818113000_feed_posted_at.sql` | `transactions.feed_posted_at` 추가 — 가족 피드 카드 머리의 시각을 체결 시각이 아니라 **글 올린 시각**으로 만든다. 이미 올라가 있는 글은 올린 때를 알 길이 없어 `created_at` 으로 채우고, `feed_body is not null` 부분 인덱스를 함께 만든다. **코드보다 먼저 적용한다** — `GET /api/family` 가 이 컬럼을 `select`·`order` 에 넣어 없으면 502 다. **아직 라이브 미적용** |

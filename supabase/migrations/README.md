...
| `20260818064500_limit_price_tick_size.sql` | 지정가 한도가격을 호가단위로 묶는다. `stocks.market`(KOSPI·KOSDAQ) 컬럼과 `tick_size(price, market)` 신설, `reserve_order` 가 호가단위에 맞지 않는 `limit_price` 를 거절한다. **적용 순서가 거꾸로다** — 아래 "배포 순서" 참고 |
| `20260818082500_drop_sell_memo.sql` | 매도 완료 화면의 한 줄 메모를 철거한다. 저장돼 있던 `side='sell'` 메모를 `null` 로 비우고 `set_trade_memo` 를 drop 한다. 적는 자리만 있고 그리는 소비자가 없었다(F2 SPEC §7.2). **`transactions.memo` 컬럼과 매수 메모는 그대로다.** **적용 완료 — 2026-08-18, 라이브(hero-kiwoom).** 매도 3행이 비워졌고 함수 1개가 사라졌다 |
| `20260818090037_reset_child_two_buys.sql` | 김찬영(1)의 거래내역을 매수 두 건(LG전자 30주 @182,000 · 농심 10주 @386,000)으로 통째로 교체하고 `rebuild_holdings(1)` 로 보유·잔액을 다시 낸다. **전량 교체다** — 요청한 현금 680,000원은 `10,000,000 − 매수` 로만 나오므로 기존 체결이 한 건이라도 남으면 어긋난다. 요청 날짜 8/12·8/14 는 일봉과 맞지 않아(8/12 LG전자는 202,500~211,500, 8/14 는 휴장) 8/10·8/11 로 옮겼다. **적용 완료 — 2026-08-18, 라이브(hero-kiwoom).** 거래 43건·부모 댓글 3개·좋아요 5개·미체결 예약 4건이 함께 사라졌다 |
| `20260818093200_feed_posted_at.sql` | `transactions.feed_posted_at` 추가 — 가족 피드 카드 머리의 시각과 정렬을 체결 시각이 아니라 **글 올린 시각**으로 만든다. 이미 올라가 있던 글은 올린 때를 알 길이 없어 `created_at` 으로 채우고, `feed_body is not null` 부분 인덱스를 함께 만든다. **코드보다 먼저 적용했다** — `GET /api/family` 가 이 컬럼을 `select`·`order` 에 넣어 없으면 502 다. **적용 완료 — 2026-08-18, 라이브(hero-kiwoom).** MCP 로 올려 remote 기록이 `20260818093200` 이라 파일 이름을 거기 맞췄다. 올라가 있던 글 20건이 체결 시각으로 채워졌다 |
| `20260818120000_create_trade_restrictions.sql` | `trade_restrictions` 신설 — 학교 시간 거래 제한(부모가 켜고 같은 `family_tag` 자녀 주문에만 걸린다). 가족 하나에 규칙 하나라 `family_tag` 가 기본키이고, 가족이 없는 계정은 `user:<id>` 를 키로 쓴다. **아직 적용 전이다** — `app/api/trade-restriction/store.ts` 가 표가 없으면 서버 메모리로 버티므로 화면은 지금도 동작하고, 적용하면 그다음 저장부터 DB 가 원본이 된다(메모리 값은 서버를 껐다 켜면 사라진다) |
...
**`20260818064500_limit_price_tick_size.sql` 만 순서가 거꾸로다 — 코드가 먼저다.**
이 마이그레이션은 새 스키마를 여는 게 아니라 `reserve_order` 의 판정을 **좁힌다**.
지금 라이브에 떠 있는 코드는 `Math.round(현재가 × (1±%))` 로 호가단위에 어긋난 값을 보내므로,
코드보다 먼저 올리면 그 순간부터 라이브의 지정가 주문이 통째로 거절된다(51종목 기준 칩 조합의 72%).
